(function(){
  ...
  function importBackup(root){
    ...
    try{ const data=JSON.parse(raw); const store=data.localStorage || data; if(!store || typeof store!=='object') throw new Error('JSON sem localStorage'); window.CRMDialog?.confirm('Importar backup e sobrescrever chaves existentes?',{title:'Importar backup',danger:true}).then(ok=>{if(!ok)return;Object.entries(store).forEach(([k,v])=>localStorage.setItem(k,String(v)));toast('Backup importado. Recarregue o CRM.')});return; }
  ...
  function bind(){
    ...
    root.addEventListener('click', function(e){
      ...
      if(action==='reset-v972'){window.CRMDialog?.confirm('Limpar apenas as preferências desta área?',{title:'Limpar preferências'}).then(ok=>{if(ok){localStorage.removeItem(KEY);applySafePreferences();render()}})}
      if(action==='clear-all-storage'){window.CRMDialog?.confirm('Isso apaga os dados locais deste CRM neste navegador. Faça backup antes.',{title:'Apagar dados locais',danger:true,confirmLabel:'Apagar'}).then(ok=>{if(ok){localStorage.clear();toast('Dados locais apagados. Recarregue a página.')}})}
