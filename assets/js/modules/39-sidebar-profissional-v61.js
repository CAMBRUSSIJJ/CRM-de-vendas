/* CRM v61 — sidebar profissional, única camada ativa de navegação */
(function(){
  'use strict';
  if(window.__CRM_V61_SIDEBAR__) return;
  ...
  function icon(name){ return '<svg class="v61-nav-svg" viewBox="0 0 24 24" aria-hidden="true">'+(PATHS[name]||PATHS.grid)+'</svg>'; }
  window.crmV61Icon = icon;

  function getSettings(){
    try{ if(window.crmV61Settings) return window.crmV61Settings.load(); }catch(e){}
    try{return JSON.parse(localStorage.getItem('crm_v61_settings')||'{}');}catch(e){return {};}
  }
  ...
  function visibleItems(){
    ...
    return NAV.filter(item=>!hidden.has(item.id));
  }
  ...
  function navButton(item){
    ...
    const cls='nav-item'+(item.action?' v61-action-item':'');
    return '<button type="button" class="'+cls+'" data-view="'+esc(item.id)+'" data-label="'+esc(item.label)+'" title="'+esc(item.label)+'" aria-label="'+esc(item.label)+'">'+icon(item.icon)+'<span class="v61-nav-text">'+esc(item.label)+'</span>'+badge+'</button>';
  }
  function buildFooter(hidden){
    ...
    return '<div class="sidebar-footer">'+
      ...
      '<button type="button" class="crm-v61-settings-btn" id="crmV61SettingsBtn" data-v61-open-settings aria-label="Configurações" title="Configurações">'+icon('settings')+'<span>Configurações</span></button>'+
      '<button type="button" class="theme-toggle" id="themeToggle" data-v61-toggle-theme aria-label="Alternar tema" title="Alternar tema">'+icon('sun')+'<span>Alternar tema</span></button>'+
    '</div>';
  }
  function renderSidebar(){
    ...
    const active=$('.view.active')?.id || localStorage.getItem('crm_v45_current_view') || 'inicio';
    sidebar.setAttribute('aria-label','Navegação principal');
    sidebar.removeAttribute('hidden');
    sidebar.innerHTML = '<div class="sidebar-brand"><div class="brand-icon">'+icon('zap')+'</div><div><div class="brand-name">Outbounder</div><div class="brand-sub">CRM de vendas</div></div></div>'+
      '<nav class="sidebar-nav">'+grouped(items).map(group=>'<div class="v61-nav-section"><div class="nav-label">'+esc(group.name)+'</div>'+group.items.map(navButton).join('')+'</div>').join('')+'</nav>'+buildFooter(hidden);
    $$('.sidebar .nav-item,.sidebar [data-view]').forEach(btn=>{
      const on=btn.dataset.view===active;
  ...
  function syncBadges(){
    try{
      const oldLead=$$('#navLeadsBadge').find(x=>!x.closest('.sidebar'));
      const newLead=$('.sidebar #navLeadsBadge');
      if(oldLead && newLead) newLead.textContent=oldLead.textContent || newLead.textContent;
      const oldChat=$$('#navChatBadge').find(x=>!x.closest('.sidebar'));
      const newChat=$('.sidebar #navChatBadge');
      if(oldChat && newChat) newChat.textContent=oldChat.textContent || newChat.textContent;
    }catch(e){}
  }
  function applyTabVisibility(){
    const st=getSettings(); const hidden=new Set(Array.isArray(st.hiddenTabs)?st.hiddenTabs:[]);
    $$('[data-view]').forEach(el=>{
      if(el.closest('#crmV61Settings')) return;
      const id=el.dataset.view;
      if(!id) return;
      el.classList.toggle('is-hidden-by-user', hidden.has(id));
      if((el.classList.contains('tab') || el.classList.contains('rail-btn') || el.classList.contains('rail-logo')) && hidden.has(id)) el.setAttribute('hidden','');
      else if(el.hasAttribute('hidden') && (el.classList.contains('tab') || el.classList.contains('rail-btn') || el.classList.contains('rail-logo'))) el.removeAttribute('hidden');
    });
    const active=$('.view.active')?.id;
    if(active && hidden.has(active)){
      const next=visibleItems().find(x=>!x.action)?.id || 'inicio';
      go(next);
    }
  }
  function forceReady(){
    doc.documentElement.classList.add('crm-ready');
    doc.body.classList.add('crm-ready','crm-v61');
    doc.body.classList.remove('loading','is-loading','app-loading');
    const app=$('#app'); if(app){app.style.visibility='visible';app.style.opacity='1'; if(app.style.display==='none') app.style.display='block';}
    $$('.loading,.loader,.preloader,.splash,.loading-overlay,[data-loading],#loading,#loader,#preloader').forEach(el=>{el.style.display='none';el.setAttribute('aria-hidden','true');});
  }
  function go(view){
    if(!view) return;
    try{ if(typeof window.setView==='function') window.setView(view); }
      ...
      $$('.view[id]').forEach(sec=>{
        const on=sec.id===view; sec.classList.toggle('active',on); sec.style.display=on?'':'none';
      });
    }
    setTimeout(()=>{
      $$('.sidebar [data-view]').forEach(btn=>{
        const on=btn.dataset.view===($('.view.active')?.id || view);
        btn.classList.toggle('active',on); if(on) btn.setAttribute('aria-current','page'); else btn.removeAttribute('aria-current');
      });
    },40);
  }
  function bindSidebar(){
    if(doc.__crmV61SidebarBound) return; doc.__crmV61SidebarBound=true;
    doc.addEventListener('click',ev=>{
      const theme=ev.target.closest('[data-v61-toggle-theme]');
      if(theme){
        ev.preventDefault();
        try{
          const st=window.crmV61Settings.load();
          st.theme = st.theme==='dark' ? 'light' : 'dark';
          window.crmV61Settings.set(st);
        }catch(e){ doc.documentElement.dataset.theme = doc.documentElement.dataset.theme==='dark'?'light':'dark'; }
        return;
      }
      const btn=ev.target.closest('.sidebar [data-view],.v61-sidebar-action[data-view]');
      if(!btn) return;
  ...
  function boot(){
    ...
    try{ if(window.crmV61Settings) window.crmV61Settings.apply(window.crmV61Settings.load()); }catch(e){}
    renderSidebar();
    [120,420,900,1600].forEach(ms=>setTimeout(()=>{forceReady();renderSidebar();},ms));
  }
